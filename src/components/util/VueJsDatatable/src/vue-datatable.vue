    ...
    methods: {
        ...
        processRows() {
            if (typeof this.data === 'function') {
                ...
                const processed_data = this.data(params, function(rows, row_count) {
                    this.setRows(rows);
            ...
            const filtered_data = this.handler.filterHandler(
                this.rows,
                this.filterBy,
                this.normalized_columns,
            );

            const sorted_data = this.handler.sortHandler(
                filtered_data,
                this.sort_by,
                this.sort_dir,
            );

            const paged_data = this.handler.paginateHandler(
                sorted_data,
                this.per_page,
                this.page,
            );
        ...
        getRowClasses(row) {
            let row_classes = this.rowClasses;
